...
cwd = os.getcwd()
data_path = cwd + '/../models/data/output/'
...
b4_label = 'Narrative 4: Economy-wide delay'
b4_style = 'dotted'
b4_color = color_list[-1]

b2_label = 'Narrative 2:'
b2_style = 'dashdot'

b3_label = 'Narrative 3:'
b3_style = 'solid'
...
for i in range(len(ds4.sector.values)):
    ...
    ax[x_counter, y_counter].set_ylim((0.6, 1.001))
...
ax[0, 3].legend(bbox_to_anchor=(0.5, -1.34), loc='lower center',
                fontsize=14)
...
# set y labels
ax[0, 0].set_ylabel("Decarbonization date index\n(1 = Efficient policy)")
ax[1, 0].set_ylabel("Decarbonization date index\n(1 = Efficient policy)")
...
if save_output:
    fig.savefig(basefile + cal + '_pfig1_decarb_dates_ind.png', dpi=400,
                bbox_inches='tight')
